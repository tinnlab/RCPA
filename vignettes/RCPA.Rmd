...
### Download Affymetrix and Agilent microarray data from GEO
...
After executing the code, by running `head(affyDataset)` command, users can expect to get the below output:
...
In the above code, the function `downloadGEO` requires users to specify which color channel should be used when downloading an Agilent dataset. When `greenOnly = FALSE`, it tells the function that both red and green channels required. Similarly, we run `head(agilDataset)` command, and get the below output:
...
other attached packages:
 [1] RCPA_0.1.1                  tidyr_1.3.0                 RCyjs_2.24.0               
